...
export function ReviewsView() {
  ...
  const getVaultPathForReview = useCallback(
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      if (activeTab === 'pending') {
        ...
        const { reviews } = await getMyReviews(user.id, organization.id, {
          asRequester: true,
          asReviewer: false,
        })
      ...
      log.error('[Reviews]', 'Error loading reviews data', { error: error })
    } finally {
      setLoading(false)
    }
  ...
  const getStatusColor = (status: ReviewStatus) => {
    switch (status) {
      case 'approved':
        return 'text-plm-success'
      case 'rejected':
        return 'text-plm-error'
      case 'kicked_back':
        return 'text-plm-warning'
      case 'cancelled':
  ...
  const getStatusIcon = (status: ReviewStatus | null) => {
    switch (status) {
      case 'approved':
        return <CheckCircle2 size={14} className="text-plm-success" />
      case 'rejected':
        return <XCircle size={14} className="text-plm-error" />
      case 'kicked_back':
        return <Undo2 size={14} className="text-plm-warning" />
      case 'cancelled':
        return <X size={14} className="text-plm-fg-muted" />
      default:
        return <Clock size={14} className="text-plm-warning" />
    }
  }
                          ...
                          <span
                            className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                              review.priority === 'urgent'
                                ? 'bg-plm-error text-white'
                                : review.priority === 'high'
                                  ? 'bg-plm-warning text-white'
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    >
                      {review.status === 'approved' ? (
                        <CheckCircle2 size={20} className="text-plm-success" />
                      ) : review.status === 'rejected' ? (
                        <XCircle size={20} className="text-plm-error" />
                      ) : review.status === 'kicked_back' ? (
                        <Undo2 size={20} className="text-plm-warning" />
                      ) : review.status === 'cancelled' ? (
                  ...
                  {review.responses && review.responses.length > 0 && (
                    <div className="mt-2 flex items-center gap-2">
                        ...
                        {review.responses.slice(0, 3).map((response) => (
                          <div key={response.id} className="relative">
                            ...
                            {response.status !== 'pending' && (
                              <div
                                className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border border-plm-bg-light flex items-center justify-center ${
                      ...
                      <span className="text-[10px] text-plm-fg-muted">
                        {review.responses.filter((r) => r.status === 'approved').length}/
                        {review.responses.length} approved
                      </span>
                    </div>
                  )}
                </div>
                ...
                {expandedReview === review.id && (
                  <div className="p-3 border-t border-plm-border bg-plm-bg space-y-3">
                    ...
                    <div className="space-y-2">
                      <p className="text-[10px] text-plm-fg-muted uppercase tracking-wide font-medium">
                        Reviewers
                      </p>
                      {review.responses?.map((response) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {response.comment && (
                              <p className="text-[10px] text-plm-fg-muted truncate">
                                "{response.comment}"
                              </p>
                            )}
                          </div>
