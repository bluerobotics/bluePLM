 ...
 */
export function DriveFileResult({
  file,
  isHighlighted,
  onSelect,
  onMouseEnter,
}: DriveFileResultProps) {
  return (
    <button
      onClick={onSelect}
    ...
    >
      <span className="text-plm-fg-muted">{getDriveFileIcon(file.mimeType)}</span>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-plm-fg truncate">{file.name}</span>
        </div>
        {file.owners?.[0]?.displayName && (
          <div className="text-xs text-plm-fg-muted truncate">{file.owners[0].displayName}</div>
        )}
      </div>
