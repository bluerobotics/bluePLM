...
export function ContributionHistory() {
  ...
  const [recentActivity, setRecentActivity] = useState<
    (ActivityRecord & { type: 'activity' | 'review' })[]
  >([])
  ...
  useEffect(() => {
    ...
    const loadActivityData = async () => {
      ...
      try {
        ...
        log.error('[Contributions]', 'Error loading contribution data', { error: error })
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const filteredData = useMemo(() => {
    ...
    activityData.forEach((day, date) => {
      const filteredActivities = day.activities.filter((a) => {
        switch (filter) {
          case 'checkins':
            return a.action === 'checkin'
          case 'checkouts':
            return a.action === 'checkout'
          case 'creates':
            return a.action === 'create'
          case 'reviews':
            return a.action.startsWith('review_')
          case 'approvals':
            return a.action === 'review_approved'
          default:
            return true
        }
      })
    ...
    <section className="space-y-4">
        ...
        <div className="bg-plm-bg rounded-lg border border-plm-border p-4">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {selectedDay.activities.map((activity) => (
              ...
              >
                <div
                  className={`p-1.5 rounded ${ACTION_COLORS[activity.action] || 'text-plm-fg-muted'}`}
                >
      ...
      <div className="bg-plm-bg rounded-lg border border-plm-border p-4">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-6 text-plm-fg-muted text-sm">No recent activity</div>
        ) : (
