...
export function useInvites(orgId: string | null) {
  ...
  const loadPendingMembers = useCallback(async () => {
    ...
    try {
      ...
      log.error('[Invites]', 'Failed to load pending members', { error: error })
    } finally {
      setPendingMembersLoading(false)
    }
  }, [orgId, setPendingMembers, setPendingMembersLoading])

  const updatePendingMember = useCallback(
    async (memberId: string, formData: PendingMemberFormData): Promise<boolean> => {
      ...
      try {
        ...
        log.error('[Invites]', 'Failed to update pending member', { error: error })
  ...
  const deletePendingMember = useCallback(
    async (memberId: string): Promise<boolean> => {
      const member = pendingMembers.find((m) => m.id === memberId)
      if (!member) return false

      try {
        const { error } = await supabase.from('pending_org_members').delete().eq('id', memberId)
  ...
  const resendInvite = useCallback(
    async (member: PendingMember): Promise<boolean> => {
      ...
      try {
        ...
        const response = await fetch(`${apiServerUrl}/auth/invite`, {
          ...
          body: JSON.stringify({
            ...
            workflow_role_ids:
              member.workflow_role_ids?.length > 0 ? member.workflow_role_ids : undefined,
            resend: true,
          }),
        })
