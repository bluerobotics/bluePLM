...
export function useJobTitles(orgId: string | null) {
  ...
  const updateJobTitle = useCallback(
    async (titleId: string, name: string, color: string, icon: string): Promise<boolean> => {
  ...
  const deleteJobTitle = useCallback(
    async (titleId: string): Promise<boolean> => {
      ...
      try {
        const { error } = await supabase.from('job_titles').delete().eq('id', titleId)
  ...
  const assignJobTitle = useCallback(
    async (targetUser: OrgUser, titleId: string | null): Promise<boolean> => {
      try {
        if (titleId) {
          ...
          updateMember(targetUser.id, {
            job_title: title
              ? { id: title.id, name: title.name, color: title.color, icon: title.icon }
              : null,
          })

          addToast(
            'success',
            `Set ${targetUser.full_name || targetUser.email}'s title to ${titleName}`,
          )
        } else {
          // Remove title
