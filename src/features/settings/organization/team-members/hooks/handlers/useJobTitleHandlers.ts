...
export function useJobTitleHandlers(params: UseJobTitleHandlersParams) {
  ...
  const handleChangeJobTitle = useCallback(
    async (targetUser: OrgUser, titleId: string | null) => {
  ...
  const handleDeleteJobTitle = useCallback(
    async (title: { id: string; name: string }) => {
  ...
  const updateJobTitleDirect = useCallback(
    async (titleId: string, name: string, color: string, icon: string) => {
  ...
  const deleteJobTitleDirect = useCallback(
