    ...
    >
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center ${anyMachineOnline ? 'bg-plm-warning/20' : 'bg-plm-error/20'}`}
          >
            {anyMachineOnline ? (
          ...
          <div>
            <h3 className="text-lg font-semibold text-plm-fg">
              {anyMachineOnline
                ...
                : 'Cannot Check In - Machine Offline'}
            </h3>
            <p className="text-sm text-plm-fg-muted">
              {filesOnDifferentMachine.length} file
              {filesOnDifferentMachine.length > 1 ? 's are' : ' is'} checked out on{' '}
              {machineNames.join(', ')}.
            </p>
          </div>
        </div>
          ...
          <div className="space-y-1">
            {filesOnDifferentMachine.slice(0, MAX_VISIBLE_FILES).map((f, i) => (
              <div key={i} className="flex items-center gap-2 text-sm">
                <File
                  size={14}
                  className={anyMachineOnline ? 'text-plm-warning' : 'text-plm-error'}
                />
        ...
        {anyMachineOnline ? (
          <div className="bg-plm-warning/10 border border-plm-warning/30 rounded p-3 mb-4">
            <p className="text-sm text-plm-fg">
              ...
              {machineNames.length === 1 ? 'that' : 'those'} computer
              {machineNames.length > 1 ? 's' : ''} will be lost.
            </p>
          ...
          <div className="bg-plm-error/10 border border-plm-error/30 rounded p-3 mb-4">
            ...
            <p className="text-xs text-plm-fg-muted mt-2">
              This ensures no unsaved work is lost. Please check in from the original computer, or
