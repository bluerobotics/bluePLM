...
export function useModalHandlers(deps: ModalHandlersDeps): UseModalHandlersReturn {
  ...
  const handleSubmitReviewRequest = useCallback(
    async (teamId?: string) => {
  ...
  const handleOpenCheckoutRequestModal = useCallback(
    (file: LocalFile) => {
  ...
  const handleOpenMentionModal = useCallback(
    async (file: LocalFile) => {
      if (!organization?.id) {
        addToast('error', 'No organization connected')
        return
  ...
  const handleToggleMentionUser = useCallback(
    (userId: string) => {
      setSelectedMentionUsers(
  ...
  const handleSubmitMention = useCallback(async () => {
    ...
    addToast(
      'info',
      `Mention noted for ${selectedMentionUsers.length} user${selectedMentionUsers.length > 1 ? 's' : ''}`,
    )
  ...
  const handleQuickShareLink = useCallback(
    async (file: LocalFile) => {
  ...
  const handleOpenECOModal = useCallback(
    async (file: LocalFile) => {
      if (!organization?.id) {
        addToast('error', 'No organization connected')
        return
