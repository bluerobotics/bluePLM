...
export function useAddFiles({
  ...
  const copyFilesWithResolution = useCallback(
    async (
  ...
  const handleAddFiles = useCallback(async () => {
    ...
    for (const file of result.files) {
      const fileName =
        (file as { relativePath?: string; name: string; path: string }).relativePath || file.name
      ...
      setConflictDialog({
        ...
        onResolve: async (resolution, _applyToAll) => {
          ...
          try {
            ...
            log.error('[AddFiles]', 'Error adding files', { error: error })
            removeToast(toastId)
    ...
    addProgressToast(
      toastId,
      `Adding ${totalFiles} file${totalFiles > 1 ? 's' : ''} to ${folderName}...`,
      totalFiles,
    )
  ...
  const handleAddFolder = useCallback(async () => {
    ...
    if (conflicts.length > 0) {
      setConflictDialog({
        ...
        onResolve: async (resolution, _applyToAll) => {
          ...
          const destFolderName = targetFolder
            ? targetFolder.split('/').pop() || targetFolder
            : 'vault root'
          addProgressToast(
            toastId,
            `Adding "${sourceFolderName}" to ${destFolderName}...`,
            totalFiles,
          )
          ...
          try {
            ...
            log.error('[AddFiles]', 'Error adding folder', { error: error })
            removeToast(toastId)
    ...
    addProgressToast(
      toastId,
      `Adding "${sourceFolderName}" (${totalFiles} file${totalFiles > 1 ? 's' : ''}) to ${destFolderName}...`,
      totalFiles,
    )
    ...
    try {
      ...
      if (errorCount === 0) {
        addToast(
          'success',
          `Added folder "${sourceFolderName}" (${successCount} file${successCount > 1 ? 's' : ''})`,
        )
      } else {
        addToast('warning', `Added ${successCount}, failed ${errorCount}`)
      }
