...
export function VersionHistoryDropdown({ file }: VersionHistoryDropdownProps) {
  ...
  useEffect(() => {
    ...
    const loadVersions = async () => {
      ...
      try {
        ...
        log.error('[VersionHistoryDropdown]', 'Failed to load versions', { error: error })
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleRollback = async (targetVersion: number) => {
    ...
    try {
      ...
      const result = await rollbackToVersion(
        ...
        targetVersion,
        isRollForward
          ? `Rolled forward to version ${targetVersion}`
  ...
  const handleSaveNote = async (version: VersionEntry) => {
    ...
    try {
      const result = await updateVersionNote(file.pdmData.id, version.id, user.id, trimmedNote)

      if (result.success) {
        setVersions((prev) =>
          prev.map((v) => (v.id === version.id ? { ...v, comment: trimmedNote || null } : v)),
        )
  ...
  const renderVersionText = () => {
    ...
    if (file.localActiveVersion !== undefined && file.localActiveVersion !== cloudVersion) {
      return (
        <span
          className="text-plm-info"
          title={`Viewing version ${file.localActiveVersion} (latest is ${cloudVersion}). Check in to save.`}
        >
    ...
    if (file.diffStatus === 'modified') {
      return (
        <span
          ...
          title={`Local changes (will be version ${cloudVersion + 1})`}
        >
  ...
  const isRolledBack =
    file.localActiveVersion !== undefined && file.localActiveVersion !== serverVersion
              ...
              {isLoading ? (
                ...
                <>
                  ...
                  {hasNewChanges && isCheckedOutByMe && !isRolledBack && (
                    <div className="p-2 rounded border bg-plm-accent/10 border-plm-accent">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="px-1 py-0.5 text-[10px] bg-plm-warning/20 text-plm-warning rounded">
                          Not checked in
                        </span>
                      </div>
                      ...
                      <div className="group/note">
                        {editingLocalNote ? (
                          ...
                          >
                            <MessageSquare
                              size={10}
                              className="text-plm-fg-muted mt-0.5 flex-shrink-0"
                            />
                            <span className="text-xs text-plm-fg-dim italic flex-1">
                              "{file.pendingCheckinNote}"
                            </span>
                            <Pencil
                              size={10}
                              className="text-plm-fg-muted opacity-0 group-hover/note:opacity-100 transition-opacity flex-shrink-0"
                            />
                          </div>
                        ) : (
                          <button
                  ...
                  {versions.map((version) => {
                    ...
                    const activeVersion =
                      file.localActiveVersion ?? file.localVersion ?? serverVersion
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          {isServerVersion && (
                            <span className="px-1 py-0.5 text-[10px] bg-plm-success/20 text-plm-success rounded">
                              Server
                            </span>
                          )}
                          {isLocalVersion && (
                          ...
                          {isLocalVersion && isRolledBack && (
                            <span className="px-1 py-0.5 text-[10px] bg-plm-info/20 text-plm-info rounded">
                              Rolled back
                            </span>
                          )}
                        ...
                        {(() => {
                          ...
                          const pendingNote = file.pendingVersionNotes?.[version.id]
                          const displayNote =
                            pendingNote !== undefined ? pendingNote : version.comment
                                ...
                                <div className="relative">
                                  ...
                                  {isSaving && (
                                    <div className="absolute right-2 top-1.5">
                                      <Loader2
                                        size={12}
                                        className="animate-spin text-plm-fg-muted"
                                      />
                                    </div>
                                  )}
                                </div>
                              ...
                              {displayNote ? (
                                ...
                                >
                                  <MessageSquare
                                    ...
                                    className="text-plm-fg-muted mt-0.5 flex-shrink-0"
                                  />
                                  <span className="text-xs text-plm-fg-dim italic flex-1 line-clamp-2">
                                    "{displayNote}"
                                  </span>
                                  {hasPendingChange && (
                                    <span
                                      className="text-[10px] text-plm-warning"
                        ...
                        {version.description && (
                          <div
                            className="text-[10px] text-plm-fg-dim mt-1 line-clamp-1"
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-[10px] text-plm-fg-muted mt-1">
                          <span title={version.created_by_user?.email}>
                            {version.created_by_user?.full_name?.split(' ')[0] ||
                              version.created_by_user?.email?.split('@')[0] ||
                              'Unknown'}
                          </span>
                          <span>•</span>
                          <span
