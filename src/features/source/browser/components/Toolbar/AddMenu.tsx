 ...
 */
export const AddMenu = memo(function AddMenu({ onAddFiles, onAddFolder }: AddMenuProps) {
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        title={
          canAdd
            ? 'Add files or folder to vault'
            : `Requires ${getPermissionRequirement('add-files')}`
        }
      >
        <Upload size={14} />
        Add
      ...
      {isOpen && canAdd && (
        ...
        >
          ...
          <div
            ...
            title={
              !canAddFolder.allowed ? `Requires ${getPermissionRequirement('add-folder')}` : ''
            }
          >
            <FolderPlus size={14} />
            Add Folder...
            {!canAddFolder.allowed && (
              <span className="text-xs text-plm-fg-muted ml-auto">(no permission)</span>
            )}
          </div>
        </div>
