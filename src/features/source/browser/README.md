...
```
browser/
├── FileBrowser.tsx           # Main orchestrator component (composes 26+ hooks)
├── types.ts                  # TypeScript interfaces and types
├── constants.ts              # Shared constants (sizes, extensions, delays)
├── index.ts                  # Barrel export (public API)
├── README.md                 # This file
├── context/
│   ├── FileBrowserContext.tsx  # Shared state provider for child components
│   └── index.ts
...
│   ├── useFileBrowserOperations.ts # Composite hook for operations
│   └── useFileBrowserView.ts     # Composite hook for view state
│
├── components/
...
The `FileBrowser.tsx` component (~1,400 lines) serves as the **orchestrator** that:
...
| Type | Purpose |
|------|---------|
| `FileBrowserProps` | Main component props |
...
```tsx
import { FileBrowser, FileBrowserProvider } from '@/features/source/browser'
...
function SourceFilesView() {
  ...
  return (
    ...
    >
      <FileBrowser onRefresh={handleRefresh} />
    </FileBrowserProvider>
  )
}
```
...
- All constants
- `FileBrowserProvider` and `useFileBrowserContext`
