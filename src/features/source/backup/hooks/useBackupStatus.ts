...
export function useBackupStatus(
  ...
  const isUsingCachedData =
    hasCachedSnapshots() && (isBackoffActive || (cacheAgeSeconds !== null && cacheAgeSeconds > 0))
  ...
  const loadSnapshots = useCallback(
    async (backupConfig: BackupConfig, forceRefresh = false) => {
      if (
        !backupConfig?.bucket ||
        !backupConfig?.access_key_encrypted ||
        !backupConfig?.restic_password_encrypted
      ) {
        return
      }
  ...
  useEffect(() => {
    ...
    loadConfig().then((loadedConfig) => {
      ...
      if (
        loadedConfig?.bucket &&
        loadedConfig?.access_key_encrypted &&
        loadedConfig?.restic_password_encrypted
      ) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (
        newConfig?.bucket &&
        newConfig?.access_key_encrypted &&
        newConfig?.restic_password_encrypted
      ) {
