...
function arePropsEqual(
  ...
  if (prevProps.item.type === 'nested-folder' || prevProps.item.type === 'folder-aggregated') {
    ...
    if (prevPath && nextPath && prevProps.isFolderSelected && nextProps.isFolderSelected) {
      if (prevProps.isFolderSelected(prevPath) !== nextProps.isFolderSelected(nextPath))
        return false
      if (
        prevProps.isFolderPartiallySelected?.(prevPath) !==
        nextProps.isFolderPartiallySelected?.(nextPath)
      )
        return false
    }
  }
...
export const VirtualizedTrashRow = memo(function VirtualizedTrashRow({
  ...
  if (type === 'folder-aggregated' && folderData) {
    ...
    const allFilesSelected =
      filesInFolder.length > 0 && filesInFolder.every((f) => selectedFiles.has(f.id))
    const someFilesSelected =
      filesInFolder.some((f) => selectedFiles.has(f.id)) && !allFilesSelected
    ...
    return (
      ...
      >
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 min-w-0 overflow-hidden">
            <div
              className="text-sm text-plm-fg truncate flex items-center gap-2"
    ...
    return (
      ...
      >
        <button
          onClick={(e) => {
            e.stopPropagation()
            onToggleFolderExpand?.(nestedFolder.path)
          }}
        ...
        <input
          ...
          ref={(el) => {
            if (el) el.indeterminate = folderPartial
          }}
        ...
        >
          <FolderOpen
            size={14}
            className={`flex-shrink-0 ${nestedFolder.recursiveCount === 0 ? 'text-plm-fg-muted/50' : 'text-plm-fg-muted'}`}
          />
          <span
            className={`text-sm truncate ${nestedFolder.recursiveCount === 0 ? 'text-plm-fg-muted' : 'text-plm-fg'}`}
            title={nestedFolder.path}
          >
            {nestedFolder.name}
          </span>
          <span className="text-xs text-plm-fg-muted">
            ({nestedFolder.recursiveCount}
            {nestedFolder.directFileCount > 0 ? ` here` : ''}
            {nestedFolder.recursiveCount === 0 ? ' - empty' : ''})
          </span>
        </div>
      </div>
    )
