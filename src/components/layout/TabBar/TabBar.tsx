...
import { X, Plus, Pin, Copy, FolderOpen } from 'lucide-react'
    ...
    >
      ...
      {isActive && (
        <>
          ...
          <div className="tab-curve-left absolute -left-[10px] bottom-0 w-[10px] h-[10px] rounded-br-full bg-transparent" />
          {/* Right interior curve */}
          <div className="tab-curve-right absolute -right-[10px] bottom-0 w-[10px] h-[10px] rounded-bl-full bg-transparent" />
        </>
      )}
      ...
      {/* Pin indicator */}
      {tab.isPinned && <Pin size={10} className="text-plm-accent flex-shrink-0" />}

      {/* Folder icon */}
      <FolderOpen size={14} className="flex-shrink-0 text-plm-accent" />

      {/* Tab title */}
      <span className="truncate text-[13px] flex-1">{tab.title}</span>
...
function TabContextMenu({ tab, position, onClose, tabCount }: TabContextMenuProps) {
  const { closeTab, closeOtherTabs, pinTab, unpinTab, duplicateTab } = usePDMStore()
...
export function TabBar() {
  const { tabs, activeTabId, tabsEnabled, addTab, moveTab } = usePDMStore()
