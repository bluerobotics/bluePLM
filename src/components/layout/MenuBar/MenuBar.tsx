...
export function MenuBar({ minimal = false }: MenuBarProps) {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        log.info(
          '[MenuBar]',
          'Sign in completed successfully, auth state change will be handled by App',
        )
      }
    } catch (error) {
      log.error('[MenuBar]', 'Sign in exception', { error: String(error) })
  ...
  return (
    <div
      ref={menuBarRef}
      className="h-[38px] bg-plm-activitybar border-b border-plm-border select-none flex-shrink-0 titlebar-drag-region grid items-center"
    ...
    >
      ...
      >
        <div className="flex items-center gap-1 px-3 titlebar-no-drag">
          {/* BluePLM Logo */}
          <div
            className="flex items-center justify-center w-7 h-7 rounded hover:bg-plm-bg-lighter transition-colors"
            title="BluePLM"
          >
            ...
            <div className="relative" ref={vaultDropdownRef}>
              ...
              >
                ...
                {visibleVaults.length > 1 && (
                  <ChevronDown
                    size={12}
                    className={`text-plm-fg-muted transition-transform ${showVaultDropdown ? 'rotate-180' : ''}`}
                  />
                )}
              </button>
                  ...
                  <div className="py-1">
                    {visibleVaults.map((vault) => (
                      ...
                      >
                        <Database
                          size={14}
                          className={`vault-icon ${vault.id === activeVaultId ? 'text-plm-accent' : 'text-plm-fg-muted'}`}
                        />
          ...
          {!minimal && topbarConfig.showZoom && (
            <div className="relative titlebar-no-drag" ref={zoomDropdownRef}>
              ...
              >
                ...
                {!zoomCondensed && (
                  <span className="text-[10px] min-w-[28px] text-center">
          ...
          {!minimal && (
            <div className="relative titlebar-no-drag" ref={topbarConfigRef}>
              ...
              {showTopbarConfigDropdown && (
                <div className="absolute right-0 top-full mt-1 w-52 bg-plm-bg-light border border-plm-border rounded-lg shadow-xl overflow-hidden z-50">
                  <div className="py-1">
                    <div className="px-3 py-1.5 text-[10px] uppercase tracking-wide text-plm-fg-dim">
                      Show in Topbar
                    </div>
                    ...
                    >
                      ...
                      <div
                        className={`w-8 h-4 rounded-full transition-colors relative ${topbarConfig.showFps ? 'bg-plm-accent' : 'bg-plm-border'}`}
                      >
                        <div
                    ...
                    >
                      ...
                      <div
                        className={`w-8 h-4 rounded-full transition-colors relative ${topbarConfig.showZoom ? 'bg-plm-accent' : 'bg-plm-border'}`}
                      >
                        <div
                          className={`absolute top-0.5 w-3 h-3 rounded-full bg-white shadow-sm transition-transform ${topbarConfig.showZoom ? 'translate-x-4' : 'translate-x-0.5'}`}
                        />
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div
                        className={`w-8 h-4 rounded-full transition-colors relative ${topbarConfig.showOrg ? 'bg-plm-accent' : 'bg-plm-border'}`}
                      >
                        <div
                          className={`absolute top-0.5 w-3 h-3 rounded-full bg-white shadow-sm transition-transform ${topbarConfig.showOrg ? 'translate-x-4' : 'translate-x-0.5'}`}
                        />
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div
                        className={`w-8 h-4 rounded-full transition-colors relative ${topbarConfig.showSearch ? 'bg-plm-accent' : 'bg-plm-border'}`}
                      >
                        <div
                          className={`absolute top-0.5 w-3 h-3 rounded-full bg-white shadow-sm transition-transform ${topbarConfig.showSearch ? 'translate-x-4' : 'translate-x-0.5'}`}
                        />
                      </div>
                    </button>
                    ...
                    <button
                      onClick={() =>
                        setTopbarConfig({ showOnlineUsers: !topbarConfig.showOnlineUsers })
                      }
                      className="w-full flex items-center gap-2.5 px-3 py-1.5 text-sm text-plm-fg hover:bg-plm-bg-lighter transition-colors"
                    >
                      <span className="text-plm-fg-muted">
                      ...
                      <div
                        className={`w-8 h-4 rounded-full transition-colors relative ${topbarConfig.showOnlineUsers ? 'bg-plm-accent' : 'bg-plm-border'}`}
                      >
                        <div
                    ...
                    <button
                      onClick={() =>
                        setTopbarConfig({ showPanelToggles: !topbarConfig.showPanelToggles })
                      }
                      className="w-full flex items-center gap-2.5 px-3 py-1.5 text-sm text-plm-fg hover:bg-plm-bg-lighter transition-colors"
                    >
                      <span className="text-plm-fg-muted">
                      ...
                      <div
                        className={`w-8 h-4 rounded-full transition-colors relative ${topbarConfig.showPanelToggles ? 'bg-plm-accent' : 'bg-plm-border'}`}
                      >
                        <div
                          className={`absolute top-0.5 w-3 h-3 rounded-full bg-white shadow-sm transition-transform ${topbarConfig.showPanelToggles ? 'translate-x-4' : 'translate-x-0.5'}`}
                        />
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div
                        className={`w-8 h-4 rounded-full transition-colors relative ${tabsEnabled ? 'bg-plm-accent' : 'bg-plm-border'}`}
                      >
                        <div
                          className={`absolute top-0.5 w-3 h-3 rounded-full bg-white shadow-sm transition-transform ${tabsEnabled ? 'translate-x-4' : 'translate-x-0.5'}`}
                        />
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div
                        className={`w-8 h-4 rounded-full transition-colors relative ${topbarConfig.showUserName ? 'bg-plm-accent' : 'bg-plm-border'}`}
                      >
                        <div
                          className={`absolute top-0.5 w-3 h-3 rounded-full bg-white shadow-sm transition-transform ${topbarConfig.showUserName ? 'translate-x-4' : 'translate-x-0.5'}`}
                        />
                      </div>
                    </button>
                  </div>
              ...
              >
                {getEffectiveAvatarUrl(user) ? (
                  <>
                    ...
                    />
                    <div
                      className={`rounded-full bg-plm-accent flex items-center justify-center text-white font-semibold hidden ${
                  ...
                  <div
                    className={`rounded-full bg-plm-accent flex items-center justify-center text-white font-semibold ${
                      showUserNameFinal ? 'w-7 h-7 text-xs' : 'w-6 h-6 text-[10px]'
                    }`}
                  >
                    {getUserInitial(user)}
                ...
                {showUserNameFinal && (
                  <>
                    ...
                    <ChevronDown
                      size={12}
                      className={`text-plm-fg-muted transition-transform ${showUserMenu ? 'rotate-180' : ''}`}
                    />
                  </>
                )}
              </button>
              ...
              {showUserMenu && (
                <div className="absolute right-0 top-full mt-1 w-56 bg-plm-bg-light border border-plm-border rounded-lg shadow-xl overflow-hidden z-50">
                      ...
                      onClick={() => {
                        ...
                        setTimeout(() => {
                          window.dispatchEvent(
                            new CustomEvent('navigate-settings-tab', { detail: 'profile' }),
                          )
                        }, 0)
                      }}
                  ...
                  <div className="px-4 py-2 border-b border-plm-border">
                    <div className="text-[10px] uppercase tracking-wide text-plm-fg-dim mb-1.5">
                      Sessions
                    </div>
                      ...
                      {sessions.map((session) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span
                              className={isCurrentDevice ? 'text-plm-accent' : 'text-plm-fg-muted'}
                            >
                              {getPlatformIcon(session.platform)}
                            </span>
                            <span
                              className={`flex-1 truncate ${isCurrentDevice ? 'text-plm-fg' : 'text-plm-fg-muted'}`}
                            >
