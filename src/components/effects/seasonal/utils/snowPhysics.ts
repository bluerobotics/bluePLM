...
export function spatialNoise(
  x: number,
  y: number,
  t: number,
  out: { dx: number; dy: number } = _spatialNoiseOut,
): { dx: number; dy: number } {
  ...
  const dx1 =
    Math.sin(x * scale1 + t * timeScale) * Math.cos(y * scale1 * 0.7 + t * timeScale * 0.8)
  const dy1 =
    Math.cos(x * scale1 * 0.8 + t * timeScale * 1.1) *
    Math.sin(y * scale1 + t * timeScale * 0.9) *
    0.3
  ...
  const dx3 =
    Math.sin(x * scale3 + t * timeScale * 5) * Math.sin(y * scale3 * 1.3 + t * timeScale * 4) * 0.25
  const dy3 =
    Math.cos(x * scale3 * 0.9 + t * timeScale * 4.5) *
    Math.cos(y * scale3 + t * timeScale * 5.5) *
    0.08
...
export function calculateWindForces(
  ...
  out.baseWindX =
    (wind.baseWind + wind.turbulence + gustForceX) * effectiveBluster + out.weatherWindX
