...
export function ImpersonationBanner() {
  const { impersonatedUser, stopUserImpersonation } = usePDMStore()
  ...
  const permCount = Object.keys(impersonatedUser.permissions).filter(
    (k) => k !== '__admin__',
  ).length
    ...
    <div className="flex items-center justify-center gap-3 px-4 py-1.5 bg-cyan-500/15 border-b border-cyan-500/30 text-cyan-200 text-sm">
      ...
      <span className="flex items-center gap-2 ml-2 text-cyan-200/50">
        <span
          ...
          title={`${teamCount} team${teamCount !== 1 ? 's' : ''}`}
        >
          <Users size={11} />
          {teamCount}
        </span>
        {vaultCount > 0 && (
          <span
            className="flex items-center gap-1"
            title={`${vaultCount} vault${vaultCount !== 1 ? 's' : ''} accessible`}
          >
            <Database size={11} />
            {vaultCount}
          </span>
        ...
        {permCount > 0 && (
          <span
            className="flex items-center gap-1"
            title={`${permCount} resource permission${permCount !== 1 ? 's' : ''}`}
          >
            <Shield size={11} />
            {permCount}
          </span>
