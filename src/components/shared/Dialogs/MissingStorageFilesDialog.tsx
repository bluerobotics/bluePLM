...
function MissingStorageFilesDialogInner({
  files,
  onClose,
  onRefresh,
}: MissingStorageFilesDialogProps) {
  const { clearMissingStorageFiles, addToast } = usePDMStore()
  const [isProcessing, setIsProcessing] = useState(false)
  const [selectedFiles, setSelectedFiles] = useState<Set<string>>(
    new Set(files.map((f) => f.fileId)),
  )
  ...
  const handleCheckoutSelected = async () => {
    ...
    try {
      ...
      if (result.succeeded > 0) {
        addToast(
          'success',
          `Checked out ${result.succeeded} file${result.succeeded > 1 ? 's' : ''}. Now check them in to re-upload.`,
        )
      ...
      if (result.failed > 0) {
        addToast(
          'warning',
          `${result.failed} file${result.failed > 1 ? 's' : ''} failed to check out`,
        )
      }
    } catch (error) {
      addToast('error', `Checkout failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
    } finally {
        ...
        <div className="px-4 py-3 bg-plm-error/5 border-b border-plm-border flex-shrink-0">
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-plm-fg">
              <p className="mb-2">
                These files show as <strong>"needs update"</strong> but the file content is missing
                from cloud storage. This usually happens when a check-in partially failed (database
                was updated but the file upload failed).
              </p>
              <p className="text-plm-fg-muted">
                <strong>To fix:</strong> Check out the files, then check them back in. This will
                re-upload the file content.
              </p>
            </div>
          </div>
        </div>
 ...
 */
export function MissingStorageFilesContainer({
  onRefresh,
}: {
  onRefresh?: (silent?: boolean) => void
}) {
