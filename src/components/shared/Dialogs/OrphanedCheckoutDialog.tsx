...
function useOrphanedActions(
  ...
  const createTempFile = (diffStatus?: 'cloud') => {
    ...
    return {
      ...
      pdmData: { id: checkout.fileId } as any, // TODO: type this
  ...
  const handleBackup = async () => {
    ...
    const ext = checkout.fileName.includes('.') ? '.' + checkout.fileName.split('.').pop() : ''
...
export function OrphanedCheckoutDialog({
  checkout,
  onClose,
  onRefresh,
}: OrphanedCheckoutDialogProps) {
      ...
      >
        <div className="flex items-start gap-3">
          <div className={`mt-0.5 ${iconClasses[action.variant]}`}>
            {isThisAction ? <Loader2 size={20} className="animate-spin" /> : <Icon size={20} />}
          </div>
          <div className="flex-1">
            <div className="font-medium text-plm-fg">{action.label}</div>
            <div className="text-sm text-plm-fg-muted">{action.description}</div>
          </div>
          {isThisAction && <Check size={16} className="text-plm-accent mt-1" />}
        </div>
      </button>
    )
  }
      ...
      <div className="bg-plm-bg-light border border-plm-border rounded-xl shadow-2xl max-w-lg w-full mx-4 overflow-hidden">
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm">
                <p className="text-plm-fg">
                  This file was <strong>checked in from {checkout.checkedInBy}</strong>. You have
                  local changes that may not match the server version.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="px-4 py-3 bg-plm-bg border-t border-plm-border">
          <p className="text-xs text-plm-fg-muted text-center">
            Server version: v{checkout.newVersion} • Checked in at{' '}
            {checkout.checkedInAt ? new Date(checkout.checkedInAt).toLocaleString() : 'unknown'}
          </p>
        </div>
      </div>
    </div>
...
function FileRow({
  ...
  return (
    <div
      className={`border border-plm-border rounded-lg overflow-hidden ${isSelected ? 'ring-2 ring-plm-accent' : ''}`}
    >
      {/* Main row */}
      <div className="flex items-center gap-2 p-2 bg-plm-bg-light">
        ...
        {/* Expand/collapse */}
        <button onClick={onToggleExpand} className="p-1 rounded hover:bg-plm-bg transition-colors">
          {isExpanded ? (
        ...
        {isThisProcessing && (
          <div className="flex items-center gap-1 text-plm-accent">
            ...
            <span className="text-xs">
              {ACTIONS.find((a) => a.key === processingAction)?.shortLabel}
            </span>
          </div>
        )}
      ...
      {isExpanded && (
        <div className="p-3 bg-plm-bg border-t border-plm-border">
          <div className="text-xs text-plm-fg-muted mb-2">
            Checked in from <span className="font-medium text-plm-fg">{checkout.checkedInBy}</span>{' '}
            at {checkout.checkedInAt ? new Date(checkout.checkedInAt).toLocaleString() : 'unknown'}{' '}
            • v{checkout.newVersion}
          </div>
 ...
 */
export function OrphanedCheckoutsContainer({
  onRefresh,
}: {
  onRefresh?: (silent?: boolean) => void
}) {
  const { orphanedCheckouts, removeOrphanedCheckout, clearOrphanedCheckouts, addToast, vaultPath } =
    usePDMStore()
  ...
  const handleSingleAction = async (checkout: OrphanedCheckout, action: ActionType) => {
    ...
    try {
      ...
      addToast(
        'success',
        `${ACTIONS.find((a) => a.key === action)?.shortLabel}: ${checkout.fileName}`,
      )
      removeOrphanedCheckout(checkout.fileId)
      setSelectedIds((prev) => {
      ...
      addToast(
        'error',
        `Failed on ${checkout.fileName}: ${error instanceof Error ? error.message : 'Unknown error'}`,
      )
    } finally {
      setIsProcessing(false)
  ...
  const handleBulkAction = async (action: ActionType) => {
    ...
    for (const checkout of selectedCheckouts) {
      ...
      try {
        ...
        failCount++
        log.error('[OrphanedCheckout]', `Failed to process ${checkout.fileName}`, { error: error })
      }
    }
    ...
    if (failCount === 0) {
      addToast(
        'success',
        `${actionName}: ${successCount} file${successCount !== 1 ? 's' : ''} processed`,
      )
    } else {
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-plm-border bg-plm-warning/5 flex-shrink-0">
          <div className="flex items-center gap-2 text-plm-warning">
            ...
            <span className="font-semibold">
              {orphanedCheckouts.length} Orphaned Checkout
              {orphanedCheckouts.length !== 1 ? 's' : ''}
            </span>
          </div>
          <button
        ...
        <div className="px-4 py-3 bg-plm-warning/5 border-b border-plm-border flex-shrink-0">
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-plm-fg">
              These files were checked in from another machine while you had local changes. Choose
              how to handle each file, or apply an action to all selected files.
            </div>
          </div>
        </div>
        ...
        {showBulkConfirm && (
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
            <div className="bg-plm-bg-light border border-plm-border rounded-lg p-4 max-w-sm mx-4 shadow-xl">
              <div className="text-plm-fg font-medium mb-2">Confirm Bulk Action</div>
              <p className="text-sm text-plm-fg-muted mb-4">
                Apply "{ACTIONS.find((a) => a.key === showBulkConfirm)?.label}" to{' '}
