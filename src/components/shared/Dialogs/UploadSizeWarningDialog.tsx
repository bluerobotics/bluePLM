      ...
      <div className="bg-plm-bg-light border border-plm-border rounded-xl shadow-2xl max-w-lg w-full mx-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-plm-border bg-plm-warning/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-semibold text-plm-fg">Large Files Detected</h2>
              <p className="text-sm text-plm-fg-muted">
                {largeFiles.length} file{largeFiles.length !== 1 ? 's' : ''} exceed
                {largeFiles.length === 1 ? 's' : ''} {thresholdMB} MB
              </p>
            </div>
          </div>
          <button
          ...
          <div className="flex items-start gap-3 p-3 bg-plm-warning/5 rounded-lg border border-plm-warning/20">
            ...
            <div className="text-sm text-plm-fg">
              <p>
                ...
                <strong>
                  {largeFiles.length} file{largeFiles.length !== 1 ? 's' : ''}
                </strong>{' '}
                larger than your {thresholdMB} MB threshold ({formatFileSize(totalLargeSize)}{' '}
                total).
              </p>
              {smallFilesCount > 0 && (
                <p className="text-plm-fg-muted mt-1">
                  {smallFilesCount} other file{smallFilesCount !== 1 ? 's' : ''} will upload
                  normally.
                </p>
              )}
            </div>
            ...
            {largeFiles.map((file, index) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-plm-fg truncate font-medium">{file.name}</div>
