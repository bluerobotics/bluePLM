...
export function useVaultManagement() {
  ...
  const handleOpenRecentVault = useCallback(
    async (path: string) => {
      ...
      if (result.success) {
        ...
        const matchingVault = currentVaults.find(
          (v) => v.localPath.toLowerCase().replace(/\\/g, '/') === normalizedPath,
        )
        if (matchingVault) {
  ...
  useEffect(() => {
    ...
    if (
      state.isLoading ||
      state.statusMessage === 'Loading organization...' ||
      state.statusMessage === 'Loading files...'
    ) {
      setIsLoading(false)
      setStatusMessage('')
    }
      ...
      if (result.success) {
        ...
        if (user || isOfflineMode) {
          ...
          if (
            ...
            result.error?.includes('Path does not exist')
          ) {
