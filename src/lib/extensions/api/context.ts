...
import type { ExtensionContextInfo, UserContext, OrganizationContext, Disposable } from './types'
    ...
    if (listeners) {
      for (const callback of listeners) {
        try {
          ...
          log.error(`[Extension:${extensionId}]`, 'Error in context change callback', { error })
        }
      }
    }
  }
 ...
 */
export function createInitialContext(extensionId: string, version: string): ExtensionContextInfo {
 ...
 */
export async function fetchContext(extensionId: string): Promise<ExtensionContextInfo> {
  try {
    const context = await sendIPC<ExtensionContextInfo>(CONTEXT_IPC_CHANNELS.GET_CONTEXT, {
      extensionId,
    })
    ...
    log.error(`[Extension:${extensionId}]`, 'Failed to fetch context', { error })
 ...
 */
export function getCachedContext(extensionId: string): ExtensionContextInfo | undefined {
 ...
 */
export function createContextProxy(extensionId: string, version: string): ExtensionContextInfo {
  ...
  return new Proxy(context, {
    get(target, prop) {
      ...
      if (cached && prop in cached) {
        return (cached as any)[prop] // TODO: type this
      }
      return (target as any)[prop] // TODO: type this
    },
  })
}
 ...
 */
export function getOrgName(context: ExtensionContextInfo, fallback: string = 'Unknown'): string {
...
export function createActivationContext(
  ...
  const extLog = {
    debug(message: string, ...args: unknown[]): void {
      log.debug(`[Extension:${extensionId}]`, message, args.length > 0 ? { args } : undefined)
    },
    info(message: string, ...args: unknown[]): void {
      log.info(`[Extension:${extensionId}]`, message, args.length > 0 ? { args } : undefined)
    },
    warn(message: string, ...args: unknown[]): void {
      log.warn(`[Extension:${extensionId}]`, message, args.length > 0 ? { args } : undefined)
    },
    error(message: string, ...args: unknown[]): void {
      log.error(`[Extension:${extensionId}]`, message, args.length > 0 ? { args } : undefined)
    },
  }
...
export function disposeActivationContext(context: ExtensionActivationContext): void {
  for (const subscription of context.subscriptions) {
    try {
      ...
      log.error(`[Extension:${context.extensionId}]`, 'Error disposing subscription', { error })
    }
  }
