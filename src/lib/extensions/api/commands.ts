...
import type { CommandsAPI, CommandHandler, CommandOptions, Disposable } from './types'
...
function validateCommandId(commandId: string, extensionId: string): void {
  ...
  const hasValidPrefix = validPrefixes.some((prefix) => commandId.startsWith(`${prefix}.`))
 ...
 */
export function handleCommandInvocation(commandId: string, args: unknown[]): unknown {
 ...
 * ```
 */
export function createCommandsAPI(extensionId: string, grantedPermissions: string[]): CommandsAPI {
  return {
    /**
    ...
    async executeCommand<T = unknown>(id: string, ...args: unknown[]): Promise<T> {
      ...
      const result = await sendIPC<{ result: T; error?: string }>(COMMANDS_IPC_CHANNELS.EXECUTE, {
        extensionId,
        commandId: id,
        args,
      })
    ...
    async getCommands(): Promise<string[]> {
      ...
      const result = await sendIPC<{ commands: string[] }>(COMMANDS_IPC_CHANNELS.GET_ALL, {
        extensionId,
      })
