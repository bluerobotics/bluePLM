...
import { installFromStore, sideloadFromFile, uninstallExtension } from './installer'
import {
  checkForUpdates,
  updateExtension,
  rollbackExtension,
...
// TIMING CONSTANTS
// ═══════════════════════════════════════════════════════════════════════════════
const EXTENSION_LOAD_MS = 100
const EXTENSION_UNLOAD_MS = 50
 ...
 */
export type ExtensionStateCallback = (
  extensionId: string,
...
export class ExtensionRegistry {
  ...
  async initialize(config: RegistryConfig): Promise<void> {
    if (this._initialized) {
      log.warn('[Registry]', 'Already initialized')
      return
    }
    ...
    if (config.autoCheckUpdates) {
      ...
      this._updateCheckTimer = setInterval(() => {
        this.checkForUpdates().catch((error) => log.error('[Registry]', 'Update check failed', { error }))
      }, interval)

      // Initial check after a short delay
      setTimeout(() => {
        this.checkForUpdates().catch((error) => log.error('[Registry]', 'Initial update check failed', { error }))
    ...
    log.info('[Registry]', `Initialized with ${this._extensions.size} extensions`)
  }
  ...
  private async _discoverLocalExtensions(): Promise<void> {
    ...
    if (result.errors.length > 0) {
      log.warn('[Registry]', 'Errors discovering extensions', { errors: result.errors })
    }
  }
  ...
  private async _activateExtension(
    ...
    try {
      ...
      if (hasClientComponent(extension.manifest)) {
        ...
        await new Promise((resolve) => setTimeout(resolve, EXTENSION_LOAD_MS))
      }
  ...
  async deactivate(extensionId: string): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      await new Promise((resolve) => setTimeout(resolve, EXTENSION_UNLOAD_MS))
    ...
    if (result.updates.length > 0) {
      ...
      for (const callback of this._updateCallbacks) {
        try {
          ...
          log.error('[Registry]', 'Error in update callback', { error })
        }
      }
    }
  ...
  private _handleStateChange(event: StateChangeEvent): void {
    ...
    for (const callback of this._stateCallbacks) {
      try {
        ...
        log.error('[Registry]', 'Error in state callback', { error })
      }
    }
  }
