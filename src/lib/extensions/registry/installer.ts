...
import type { PackageContents, VerificationStatus } from '../types'
import {
  extractPackage,
  verifyPackageHash,
...
export async function uninstallExtension(
  ...
  try {
    ...
    if (window.electronAPI?.deleteDirectory) {
      // @ts-expect-error - deleteDirectory will be added by Agent 5
      const result = (await window.electronAPI.deleteDirectory(extensionPath)) as {
        success: boolean
        error?: string
      }
      if (!result.success) {
...
async function installLocally(
  ...
  if (window.electronAPI.createDirectory) {
    // @ts-expect-error - createDirectory will be added by Agent 5
    const result = (await window.electronAPI.createDirectory(extensionDir)) as {
      success: boolean
      error?: string
    }
    if (!result.success) {
...
async function deployServerHandlers(
  ...
  const routes =
    manifest.contributes.apiRoutes?.map((route) => ({
      method: route.method,
      path: route.path,
