...
export async function discoverLocalExtensions(
  ...
  if (!electronAPI?.listDirectory) {
    log.warn('[Discovery]', 'Local discovery requires Electron environment')
  ...
  try {
    ...
    log.error('[Discovery]', 'Failed to discover local extensions', { error })
    errors.push({
      path: extensionsPath,
      error: error instanceof Error ? error.message : String(error),
    })
...
export async function discoverStoreExtensions(
  ...
  try {
    ...
    const extensions = data.extensions.map((ext) => ({
      ...
      deprecation: ext.deprecation
        ? {
            ...
            sunsetDate: ext.deprecation.sunsetDate
              ? new Date(ext.deprecation.sunsetDate)
              : undefined,
          }
        : undefined,
    }))
    ...
    log.error('[Discovery]', 'Failed to fetch store extensions', { error })

    // Return cached data if available, even if stale
    if (storeCache) {
      log.warn('[Discovery]', 'Using stale cache due to error')
      return {
...
export async function getFeaturedExtensions(
  ...
  try {
    ...
    log.error('[Discovery]', 'Failed to fetch featured extensions', { error })
    return []
  }
}
...
export async function getStoreExtension(
  ...
  try {
    const response = await fetch(
      `${storeApiUrl}/store/extensions/${encodeURIComponent(extensionId)}`,
    )
    ...
    log.error('[Discovery]', 'Failed to fetch extension', { error })
    return null
  }
}
...
export async function getExtensionVersions(
  ...
  try {
    const response = await fetch(
      `${storeApiUrl}/store/extensions/${encodeURIComponent(extensionId)}/versions`,
    )
    ...
    log.error('[Discovery]', 'Failed to fetch versions', { error })
    return []
  }
}
...
export async function searchExtensions(
  ...
  const { includeLocal = true, includeStore = true, storeApiUrl = DEFAULT_STORE_API_URL } = options
