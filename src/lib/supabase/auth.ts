  ...
  return routeBackend({
    ...
    supabase: async () => {
      ...
      if (isElectron) {
        ...
        return {
          data: null,
          error: result?.canceled ? null : new Error('Sign in was not completed'),
        }
      }
  ...
  return routeBackend({
    ...
    supabase: async () => {
      ...
      try {
        ...
        authLog('error', 'signInWithEmail exception', { error: String(error) })
        return { data: null, error: error as Error }
      }
    },
  })
  ...
  return routeBackend({
    ...
    supabase: async () => {
      ...
      try {
        ...
        authLog('error', 'signUpWithEmail exception', { error: String(error) })
        return { data: null, error: error as Error }
      }
    },
  })
...
export async function signInWithPhone(phone: string) {
  return routeBackend({
    ...
    supabase: async () => {
      const client = getSupabaseClient()
      authLog('info', 'signInWithPhone called - sending OTP', {
      ...
      try {
        ...
        authLog('error', 'signInWithPhone exception', { error: String(error) })
        return { data: null, error: error as Error }
      }
    },
  })
  ...
  return routeBackend({
    ...
    supabase: async () => {
      ...
      try {
        ...
        authLog('info', 'Phone OTP verified successfully', {
          userId: data.user?.id?.substring(0, 8),
        })
        return { data, error: null }
      } catch (error) {
        authLog('error', 'verifyPhoneOTP exception', { error: String(error) })
        return { data: null, error: error as Error }
      }
    },
  })
...
export async function checkIfSupplierAccount(identifier: string): Promise<{
  ...
  authLog('info', 'checkIfSupplierAccount called', {
  ...
  try {
    ...
    authLog('error', 'checkIfSupplierAccount failed', { error: String(error) })
    return { isSupplier: false }
  }
}
...
export async function getSupplierContact(authUserId: string) {
  ...
  try {
    ...
    authLog('error', 'getSupplierContact failed', { error: String(error) })
