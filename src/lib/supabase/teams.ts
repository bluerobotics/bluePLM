...
export async function updateUserRole(
  ...
  // Update the role
  const { error } = await client.from('users').update({ role: newRole }).eq('id', targetUserId)
...
export async function addUserToOrg(
  ...
  if (fetchError || !existingUser) {
    return {
      success: false,
      error: 'No user found with that email. They must sign in to BluePLM at least once first.',
    }
  }
 ...
 */
export async function loadImpersonatedUserContext(targetUserId: string): Promise<{
  user: {
    id: string
    email: string
