 ...
 * // returns 'A-01'
 */
export function validateTabInput(
  value: string,
  options: TabValidationOptions | number = DEFAULT_TAB_VALIDATION_OPTIONS,
): string {
  // Handle backwards compatibility: if options is a number, treat it as maxLength with defaults
  const opts: TabValidationOptions =
 ...
 */
export function sanitizeTabNumber(
  value: string | null | undefined,
  options?: TabValidationOptions,
): string {
  if (!value) return ''
 ...
 */
export function getTabPlaceholder(
  options: TabValidationOptions | number = DEFAULT_TAB_VALIDATION_OPTIONS,
): string {
 ...
 */
export function getTabValidationOptions(
  settings:
    | {
