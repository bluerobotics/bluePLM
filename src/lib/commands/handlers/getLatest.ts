...
export function getOutdatedFilesFromSelection(
  files: LocalFile[],
  selection: LocalFile[],
): LocalFile[] {
  ...
  logGetLatest('debug', 'getOutdatedFilesFromSelection called', {
    ...
    selectionPaths: selection.map((f) => ({
      path: f.relativePath,
...
export const getLatestCommand: Command<GetLatestParams> = {
  id: 'get-latest' as any, // TODO: type this
  name: 'Get Latest',
  ...
  async execute({ files }, ctx): Promise<CommandResult> {
    ...
    if (outdatedFiles.length === 0) {
      logGetLatest('warn', 'No outdated files found - returning early', {
        operationId,
        inputFilesCount: files.length,
        inputFileStatuses: files.map((f) => ({
          name: f.name,
          diffStatus: f.diffStatus,
          hasPdmData: !!f.pdmData,
        })),
      })
    ...
    const objectNotFoundErrors = errors.filter(
      (e) =>
    ...
    const hasOnlyMissingStorageIssues =
      totalFailed > 0 && totalFailed === missingStorageFiles.length + objectNotFoundErrors.length
    ...
    if (succeeded > 0 && ctx.activeVaultId) {
      const updatedPaths = pendingUpdates
        .map((u) => {
      ...
      if (updatedPaths.length > 0) {
        addToSyncIndex(ctx.activeVaultId, updatedPaths).catch((error) => {
          logGetLatest('warn', 'Failed to update sync index after get-latest', {
            error: String(error),
          })
        })
      }
    ...
    tracker.endOperation(
      ...
      totalFailed > 0 ? errors[0] : undefined,
    )
