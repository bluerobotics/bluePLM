 ...
 */
function getSyncedFilesWithReferences(
  files: LocalFile[],
  selection: LocalFile[],
): SyncedFileInfo[] {
...
export const extractReferencesCommand: Command<ExtractReferencesParams> = {
  ...
  description:
    'Extract and store file references (assemblies and drawings) to database for Contains/Where-Used queries',
  ...
  async execute({ files }, ctx): Promise<CommandResult> {
    ...
    const assembliesCount = filesWithRefs.filter(
      (f) => f.extension.toLowerCase() === '.sldasm',
    ).length
    const drawingsCount = filesWithRefs.filter(
    ...
    if (!status?.data?.running) {
      ...
      ctx.addToast(
        'error',
        'SolidWorks service is not running. Please start SolidWorks and try again.',
      )
      ...
      try {
        ...
        if (!result?.success) {
          ...
          if (result?.error?.includes('access') || result?.error?.includes('locked')) {
            ...
            // Unknown failure
            details.push(
              `${file.fileName}: Could not read references (file may need to be opened in SolidWorks first)`,
            )
            skipped++
          }
          progress.update()
        ...
        const swRefs = result.data?.references as
          | Array<{
              path: string
              fileName: string
              exists: boolean
    ...
    if (failed > 0) {
      ...
      ctx.addToast(
        'warning',
        `Extracted references: ${succeeded} succeeded, ${failed} failed${skipped > 0 ? `, ${skipped} skipped` : ''}${failedMsg}`,
      )
    } else if (succeeded > 0) {
      ctx.addToast(
        'success',
        `Extracted references from ${succeeded} file${succeeded > 1 ? 's' : ''}${skipped > 0 ? ` (${skipped} skipped)` : ''}`,
      )
    } else if (skipped > 0) {
      // All files were skipped
      ctx.addToast(
        'info',
        `No references found in ${skipped} file${skipped > 1 ? 's' : ''}. These may be empty assemblies/drawings or have no synced components.`,
      )
    } else {
