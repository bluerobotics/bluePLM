...
import { getTerminalCommandHandler, getAllTerminalCommandAliases } from './registry'
...
export async function executeTerminalCommand(
  ...
  if (handler) {
    try {
      ...
      addOutput('error', `Command failed: ${error instanceof Error ? error.message : String(error)}`)
    }
    return outputs
  }
  ...
  addOutput(
    'info',
    `Processing ${filesToProcess.length} file${filesToProcess.length > 1 ? 's' : ''}...`,
  )
  ...
  try {
    ...
    switch (commandId) {
      ...
      case 'open':
        if (filesToProcess.length === 1) {
          ...
          result = {
            success: false,
            message: 'Can only open one file at a time',
            total: 0,
            succeeded: 0,
            failed: 1,
          }
      ...
      case 'show-in-explorer':
        if (filesToProcess.length === 1) {
          ...
          result = {
            success: false,
            message: 'Can only reveal one file at a time',
            total: 0,
            succeeded: 0,
            failed: 1,
          }
      ...
      default:
        result = {
          success: false,
          message: `Command not implemented: ${commandId}`,
          total: 0,
          succeeded: 0,
          failed: 1,
        }
    ...
    if (result.errors && result.errors.length > 0) {
      result.errors.slice(0, 5).forEach((error) => addOutput('error', `  ${error}`))
    }
  } catch (error) {
    addOutput('error', `Command failed: ${error instanceof Error ? error.message : String(error)}`)
  }
...
export function getAutocompleteSuggestions(input: string, files: LocalFile[]): string[] {
  ...
  if (!parsed.command || (parsed.args.length === 0 && !input.includes(' '))) {
    ...
    const pdmCommands = [
      ...
      'show',
      'clear',
      'cls', // These are handled specially
    ]

    const allCommands = [...new Set([...terminalCommands, ...pdmCommands])]
    return allCommands.filter((c) => c.startsWith(parsed.command || '')).slice(0, 30)
  }
