...
> = (set) => ({
  ...
  addJobTitle: (title: JobTitle) =>
    set((state) => ({
      jobTitles: [...state.jobTitles, title],
    })),

  updateJobTitleInStore: (id: string, updates: Partial<JobTitle>) =>
    set((state) => ({
      jobTitles: state.jobTitles.map((t) => (t.id === id ? { ...t, ...updates } : t)),
    })),

  removeJobTitle: (id: string) =>
    set((state) => ({
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  setWorkflowRoles: (roles: WorkflowRoleBasic[]) =>
    set({
      workflowRoles: roles,
  ...
  setUserRoleAssignments: (assignments: Record<string, string[]>) =>
    set({
      userRoleAssignments: assignments,
    }),

  addWorkflowRole: (role: WorkflowRoleBasic) =>
    set((state) => ({
      workflowRoles: [...state.workflowRoles, role],
    })),

  updateWorkflowRoleInStore: (id: string, updates: Partial<WorkflowRoleBasic>) =>
    set((state) => ({
      workflowRoles: state.workflowRoles.map((r) => (r.id === id ? { ...r, ...updates } : r)),
    })),

  removeWorkflowRole: (id: string) =>
    set((state) => ({
  ...
  setVaultAccessMap: (map: Record<string, string[]>) =>
    set({
      vaultAccessMap: map,
    }),

  setTeamVaultAccessMap: (map: Record<string, string[]>) =>
    set({
      teamVaultAccessMap: map,
    }),

  grantUserVaultAccess: (userId: string, vaultId: string) =>
    set((state) => {
  ...
  revokeUserVaultAccess: (userId: string, vaultId: string) =>
    set((state) => {
      const currentUsers = state.vaultAccessMap[vaultId] || []
  ...
  grantTeamVaultAccess: (teamId: string, vaultId: string) =>
    set((state) => {
  ...
  revokeTeamVaultAccess: (teamId: string, vaultId: string) =>
    set((state) => {
      const currentVaults = state.teamVaultAccessMap[teamId] || []
