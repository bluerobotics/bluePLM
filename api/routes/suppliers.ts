...
const supplierRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      // Get file info
      const { data: file, error: fileError } = await request
        .supabase!.from('files')
  ...
  fastify.get(
    '/parts/:id/costing',
    {
      schema: {
        description:
          'Get complete costing info for a part including all suppliers and volume pricing',
        tags: ['ERP', 'Suppliers'],
        security: [{ bearerAuth: [] }],
        params: {
          type: 'object',
        ...
        querystring: {
          ...
          properties: {
            quantity: {
              type: 'integer',
              default: 1,
              description: 'Quantity to calculate pricing for',
            },
          },
        },
      },
    ...
    async (request, reply) => {
      ...
      // Get part info
      const { data: part, error: partError } = await request
        .supabase!.from('files')
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const { data: supplier, error: supplierError } = await request
        .supabase!.from('suppliers')
        .select('id, name, code')
        .eq('id', id)
        .eq('org_id', request.user!.org_id)
