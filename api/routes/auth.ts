...
const authRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post(
    '/auth/invite',
    {
      schema: {
        description:
          'Invite a user by email. Requires admin role. Creates pending org member and sends invite email. Use resend=true to resend an invite.',
        tags: ['Auth'],
        security: [{ bearerAuth: [] }],
        body: {
          type: 'object',
          ...
          properties: {
            ...
            resend: {
              type: 'boolean',
              description: 'If true, resends invite for existing pending member',
            },
          },
        },
        response: {
    ...
    async (request, reply) => {
      ...
      if (existingAuthUser && !existingUser) {
        ...
        if (deleteError) {
          fastify.log.warn(
            { email: normalizedEmail, error: deleteError },
            'Failed to delete stale auth user',
          )
        }
      }
      ...
      if (inviteError) {
        fastify.log.warn(
          { email: normalizedEmail, error: inviteError },
          'Failed to send invite email, but pending member created',
        )
        return {
          success: true,
