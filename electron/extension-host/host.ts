...
export class ExtensionHost {
  ...
  async shutdown(): Promise<void> {
    ...
    for (const ext of extensions) {
      try {
        ...
        this.log('warn', `Error deactivating ${ext.id}`, { error: String(error) })
      }
    }
  ...
  private setupMessageHandlers(): void {
    ...
    this.ipc.on('extension:load', async (msg) => {
      ...
      try {
        ...
        this.ipc.sendExtensionError(extensionId, message)
      }
    })
    ...
    this.ipc.on('api:call', async (msg) => {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : String(error)
        this.ipc.sendApiError(callId, message)
      }
    })
  ...
  private log(level: string, message: string, data?: unknown): void {
    switch (level) {
      case 'debug':
        hostLog.debug('[ExtensionHost]', message, data)
        break
      case 'warn':
        hostLog.warn('[ExtensionHost]', message, data)
        break
      case 'error':
        hostLog.error('[ExtensionHost]', message, data)
        break
      default:
        hostLog.info('[ExtensionHost]', message, data)
    }
  }
}
...
export function initializeExtensionHost(): ExtensionHost | null {
  ...
  if (typeof window === 'undefined') {
    hostLog.error('[ExtensionHost]', 'Must run in a browser window context')
    return null
  }

  // Get the IPC bridge from preload
  const hostBridge = (window as unknown as { extensionHostBridge?: ExtensionHostBridge })
    .extensionHostBridge
  if (!hostBridge) {
    hostLog.error('[ExtensionHost]', 'Bridge not available')
    return null
  }

  // Create the host
  const host = createExtensionHost((message) => hostBridge.send(message), {
    watchdogConfig: {
      memoryLimitMB: 50,
      cpuTimeoutMs: 5000,
  ...
  window.onerror = (message, source, lineno, colno, error) => {
    hostLog.error('[ExtensionHost]', `Uncaught error: ${message}`, error)
    hostBridge.send({
      type: 'host:crashed',
  ...
  window.onunhandledrejection = (event) => {
    hostLog.error('[ExtensionHost]', 'Unhandled rejection', event.reason)
    hostBridge.send({
      type: 'host:crashed',
